...
func linktwo(w http.ResponseWriter, r *http.Request) {
	thisuser2 := r.FormValue("name") + " " + r.FormValue("email") + " " + r.FormValue("message")
	err := mytemp.ExecuteTemplate(w, "link1.html", thisuser2)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}
func linkthree(w http.ResponseWriter, r *http.Request) {
	thisuser3 := r.FormValue("name") + " " + r.FormValue("email") + " " + r.FormValue("message")
	err := mytemp.ExecuteTemplate(w, "link1.html", thisuser3)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}
